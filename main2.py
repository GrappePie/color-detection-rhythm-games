...
class ColorDetectionThread(QtCore.QThread):
        ...
        while self.running:
            with QtCore.QMutexLocker(self.mutex):
                if not self.label.active:
                    self.color_detected.emit(False, False)
                    self.msleep(10)
                    continue

            # Solo realiza detección si la zona está activa
            detected = self.detect_color()
            self.color_detected.emit(detected, True)
    ...
    def detect_color(self):
        ...
        screenshot = screen.grabWindow(0, self.label.x() + 8, self.label.y() + 8,
                                       self.label.width() - 13, self.label.height() - 13)
...
class TransparentWindow(QtWidgets.QWidget):
    ...
    def update_labels_position(self):
        for i, label in enumerate(self.labels):
            if keyboard.is_pressed(f'shift+{i + 1}') and label.active:
                self.move_label_to_cursor(i)
    ...
    def toggle_activation(self):
        ...
        try:
            for label in self.labels:
                ...
                label.setStyleSheet(
                    f"border: 5px solid {'white' if label.active else 'black'};"
                )
        finally:
            self.mutex.unlock()
